...
def setup_events(bot: commands.Bot):
    ...
    async def on_ready():
        ...
        Consumes command line arguments and writes persistent data to disk.'''
        if len(sys.argv) >= 2:
            channel_id=int(sys.argv[1])
            chx = bot.get_channel(channel_id)
    ...
    async def on_message(message):
        ...
        if not message.author.bot:
            ...
            if message.content.lower() == "cancel":
                sm_cog = bot.get_cog("SM213")
                if isinstance(sm_cog, SM213):
                    sm_cog.queue.append([message.author.id, time.time()])
